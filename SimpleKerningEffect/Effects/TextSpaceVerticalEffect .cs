...
{
    [VideoEffect("문자 간격 조정(세로쓰기)", ["텍스트"], ["kerning", "문자", "텍스트", "text", "플러그인", "plugin"], isAviUtlSupported: false, isEffectItemSupported: false)]
    internal class TextSpaceVerticalEffect : VideoEffectBase
    {
        public override string Label => $"문자 간격 조정(세로쓰기) {Space.GetValue(0, 1, 30):F1}";

        [Display(GroupName = "문자 간격 조정(세로쓰기)", Name = "시작", Description = "조정할 문자의 시작 위치")]
        ...
        [Display(GroupName = "문자 간격 조정(세로쓰기)", Name = "종료", Description = "조정할 문자의 종료 위치")]
        [TextBoxSlider("F0", "", 1, 5)]
        [DefaultValue(1d)]
        [Range(1, 99999)]
        ...
        [Display(GroupName = "문자 간격 조정(세로쓰기)", Name = "문자 간격", Description = "문자 간격")]
        [AnimationSlider("F1", "", -50, 50)]
        public Animation Space { get; } = new Animation(0, -99999, 99999);

        [Display(GroupName = "문자 간격 조정(세로쓰기)", Name = "전체 조정", Description = "범위 밖의 문자 위치를 조정합니다.")]
        [ToggleSlider]
